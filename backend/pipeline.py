...
def _call_with_config(prompt: str, llm_config: Optional[LLMConfig]) -> str:
    cfg = llm_config or {}
    return call_llm(
        prompt,
        model=cfg.get("model"),
...
def parse_resume_and_job(
    ...
    *,
...
) -> Tuple[Profile, Profile, Optional[str]]:
    prompt = prompts.build_parse_profile_prompt(resume_text, jd_text)
    raw = _call_with_config(prompt, llm_config)
    data = _loads(raw)
    try:
    ...
    return (resume_profile, job_profile, raw) if return_raw else (resume_profile, job_profile, None)


def parse_resume_only(resume_text: str, *, llm_config: Optional[LLMConfig] = None) -> Profile:
    resume_profile, _, _ = parse_resume_and_job(
...
def parse_job_only(jd_text: str, *, llm_config: Optional[LLMConfig] = None) -> Profile:
    _, job_profile, _ = parse_resume_and_job(
...
def analyze_gaps_and_mapping(
    ...
    *,
    llm_config: Optional[LLMConfig] = None,
    return_raw: bool = False,
) -> Tuple[GapAnalysisResult, JDMappingMatrix, Optional[str]]:
...
def generate_learning_plan(
    ...
    *,
    ...
    return plan, raw if return_raw else None
...
def generate_custom_resume(
    ...
    *,
    ...
    return custom_md, raw if return_raw else None
...
def run_full_analysis(
    ...
    *,
    ...
    resume_profile, job_profile, _ = parse_resume_and_job(
        resume_text, jd_text, llm_config=llm_config
    )
    gap_analysis, jd_mapping, _ = analyze_gaps_and_mapping(
        resume_profile, job_profile, llm_config=llm_config
    )
    learning_plan, _ = generate_learning_plan(
        gap_analysis, llm_config=llm_config
    )
    custom_resume_markdown, _ = generate_custom_resume(
        resume_text, jd_text, llm_config=llm_config
    )
