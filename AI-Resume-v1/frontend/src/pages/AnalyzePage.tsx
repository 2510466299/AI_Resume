...
const formatReasoning = (stage: string, raw: string): string => {
  const clean = stripCodeFence(raw);
  return `【推理·${stage}】${clean.slice(0, 600)}${clean.length > 600 ? "..." : ""}`;
};
...
function AnalyzePage({
  ...
  const [reasoningMode, setReasoningMode] = usePersistentState("reasoningMode", false);
  ...
  const streamAnalysis = async (runId: string) => {
    ...
    if (reasoningMode) {
      body.llm_model = DEFAULT_REASONING_MODEL;
    } else if (!useDefaultModel) {
    ...
    enqueueLog(runId, "流式连接已建立，等待模型响应...");
  ...
  const processSseChunk = (runId: string, chunk: string) => {
    ...
    switch (eventName) {
      ...
      case "reasoning_output":
        enqueueLog(runId, formatReasoning(data.stage, data.content));
        break;
      case "result": {
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-3 flex items-center gap-3 text-sm">
          <label className="flex items-center gap-2">
            ...
            />
            启用推理模式（deepseek-reasoner）
          </label>
          <span className="text-xs text-slate-500">
            推理模式会请求 deepseek-reasoner 并返回 reasoning_content（日志面板可查看）。
          </span>
        </div>
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          <label className="flex flex-col gap-1 text-sm">
            ...
            <input
              ...
              disabled={useDefaultModel || reasoningMode}
              onChange={(e) =>
        ...
        <p className="mt-2 text-xs text-slate-500">
          选择“默认模型”即使用 DeepSeek V3（服务器端维护的 Key），选“自定义模型”可输入自己的 Key 与 Endpoint。勾选“推理模式”时自动使用 deepseek-reasoner，可继续使用默认 Key 或自定义接口。
        </p>
      </section>
